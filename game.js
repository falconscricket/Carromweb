...
const PIECE_R = 21;
const STRIKER_R = 26;
const POCKET_R = 36;           // capture radius — slightly larger than the visual hole so near-misses still feel fair
const POCKET_MAGNET_R = 46;    // pieces drifting slowly this close get gently pulled in, like real momentum near a hole
  ...
  for(let s=0; s<SUBSTEPS; s++){
    ...
    all.forEach(p=>{
      ...
      for(const pc of pocketCenters){
        ...
        if(dist < POCKET_MAGNET_R && speed < 2.2){
          const pull = 0.35;
...
function drawBoard(){
  ...
  // outer frame (deep plum, like a lacquered rim)
  ...
  // wood base (playfield)
  const g = ctx.createRadialGradient(SIZE*0.4,SIZE*0.35,40, SIZE/2,SIZE/2, SIZE*0.75);
  g.addColorStop(0,'#e6ae6c');
  g.addColorStop(0.45,'#cf8f4c');
  g.addColorStop(0.8,'#a96f36');
  g.addColorStop(1,'#7d4f24');
  ctx.fillStyle = g;
  roundRectPath(PAD-6,PAD-6,INNER+12,INNER+12,10);
  ctx.fill();
  ...
  ctx.globalAlpha = 1;
  // soft vignette
  const vg = ctx.createRadialGradient(SIZE/2,SIZE/2, INNER*0.25, SIZE/2,SIZE/2, INNER*0.72);
  vg.addColorStop(0,'rgba(0,0,0,0)');
  vg.addColorStop(1,'rgba(30,8,20,0.28)');
  ctx.fillStyle = vg;
  ctx.fillRect(0,0,SIZE,SIZE);
  ctx.restore();

  // gold rounded rim around playfield
  ctx.lineWidth = 4;
  ctx.strokeStyle = 'rgba(232,193,112,0.7)';
  roundRectPath(PAD-6,PAD-6,INNER+12,INNER+12,10);
  ctx.stroke();

  // double guide border lines near edge (classic carrom track)
  ctx.strokeStyle = 'rgba(90,35,25,0.45)';
  ctx.lineWidth = 2.4;
  const inset1 = 26, inset2 = 40;
  ctx.strokeRect(PAD+inset1, PAD+inset1, INNER-inset1*2, INNER-inset1*2);
  ctx.strokeStyle = 'rgba(90,35,25,0.28)';
  ctx.strokeRect(PAD+inset2, PAD+inset2, INNER-inset2*2, INNER-inset2*2);

  // corner arrow markers + small guide circles (like reference board)
  const cornerSpots = [
    {x:PAD, y:PAD, dx:1, dy:1}, {x:SIZE-PAD, y:PAD, dx:-1, dy:1},
    {x:PAD, y:SIZE-PAD, dx:1, dy:-1}, {x:SIZE-PAD, y:SIZE-PAD, dx:-1, dy:-1}
  ];
  cornerSpots.forEach(c=>{
    [0.135, 0.30].forEach(o=>{
      const gx = c.x + c.dx*INNER*o, gy = c.y + c.dy*INNER*o;
      ctx.beginPath();
      ctx.arc(gx,gy, 11, 0, Math.PI*2);
      ctx.strokeStyle = 'rgba(232,193,112,0.6)';
      ctx.lineWidth = 2;
      ctx.stroke();
      ctx.fillStyle = 'rgba(212,132,159,0.12)';
      ctx.fill();
    });
  });

  // center mandala (rangoli style)
  ctx.save();
  ...
  ctx.strokeStyle = 'rgba(212,132,159,0.35)';
  for(let i=0;i<16;i++){
    ctx.rotate(Math.PI/8);
    ctx.beginPath();
    ctx.moveTo(0,-102); ctx.lineTo(0,-150);
    ctx.stroke();
  }
  ...
  for(let i=0;i<8;i++){
    const a = i*Math.PI/4;
    const rr = i%2===0 ? 100 : 60;
    ctx.lineTo(Math.cos(a)*rr, Math.sin(a)*rr);
  }
  ctx.closePath(); ctx.fill();
  ctx.restore();

  // pockets — rounded gold-yellow holders with black hole, like reference corners
  pocketCenters.forEach(pc=>{
    // yellow rounded holder
    ctx.beginPath();
    ctx.arc(pc.x, pc.y, POCKET_R+7, 0, Math.PI*2);
    const holderG = ctx.createRadialGradient(pc.x-8,pc.y-8,2, pc.x,pc.y,POCKET_R+7);
    holderG.addColorStop(0,'#ffe9a8');
    holderG.addColorStop(0.6,'#e8c170');
    holderG.addColorStop(1,'#b98a3c');
    ctx.fillStyle = holderG;
    ctx.fill();

    const pg = ctx.createRadialGradient(pc.x-6,pc.y-6,2,pc.x,pc.y,POCKET_R);
    pg.addColorStop(0,'#454545');
    pg.addColorStop(0.6,'#151515');
  ...
  // baselines — both ends, whichever player is up gets a glowing lane
  [ {y: SIZE-PAD-60, active: turn===1}, {y: PAD+60, active: turn===2} ].forEach(bl=>{
    ctx.strokeStyle = bl.active ? 'rgba(232,193,112,0.95)' : 'rgba(232,193,112,0.4)';
    ...
    if(bl.active){
      ...
      ctx.fillStyle = 'rgba(232,193,112,0.85)';
      ctx.beginPath();
...
function drawPiece(p){
  ...
  ctx.beginPath();
  ctx.ellipse(p.x+2, p.y+5, p.r*0.95, p.r*0.62, 0, 0, Math.PI*2);
  ctx.fillStyle = 'rgba(15,4,12,0.35)';
  ctx.fill();
  ...
  // main flat face
  ctx.beginPath();
  ctx.arc(p.x,p.y,p.r*0.86,0,Math.PI*2);
  const rg = ctx.createRadialGradient(p.x-p.r*0.28,p.y-p.r*0.32,p.r*0.08, p.x,p.y,p.r*0.9);
  ...
  ctx.beginPath();
  ctx.ellipse(p.x-p.r*0.28, p.y-p.r*0.34, p.r*0.34, p.r*0.18, -0.5, 0, Math.PI*2);
...
function drawStrikerDisc(p){
  ...
  const rg = ctx.createRadialGradient(p.x-p.r*0.25,p.y-p.r*0.3,p.r*0.08, p.x,p.y,p.r*0.75);
  rg.addColorStop(0, lightC); rg.addColorStop(0.55, baseC); rg.addColorStop(1, edgeC);
  ctx.fillStyle = rg;
  ctx.fill();
  // heart engraving
  drawHeart(p.x, p.y, p.r*0.42, pink ? '#d97ea4' : '#e8b8c8', pink ? '#a5324f' : '#c98fa0');
  // gloss streak
  ctx.beginPath();
  ctx.ellipse(p.x-p.r*0.22, p.y-p.r*0.28, p.r*0.28, p.r*0.15, -0.5, 0, Math.PI*2);
